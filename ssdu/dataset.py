"""Multi-contrast fastMRI dataset, collate and batch-transfer helpers."""
...
from .fft_utils import crop_kspace, ifft2c, rss
...
class FastMRISSDUMultiContrast(Dataset):
    """
    Ground truth (test only) is RSS-independent: rss(ifft2c(full_kspace)),
    no sensitivity map. The returned "sens" is used only for the SENSE
    zero-filled baseline metric.
    """
        ...
        for files in self.files_per_contrast:
            ...
            for fid, path in enumerate(files):
                ...
                if n_coils != CFG.ORIGINAL_COILS:
                    raise RuntimeError(
                        f"{path}: found {n_coils} coils; expected {CFG.ORIGINAL_COILS}."
                    )

                start = CFG.SKIP_START_SLICES
                end = max(start, n - CFG.SKIP_END_SLICES)
    ...
    def _load_one_contrast(self, c, idx):
        ...
        fid, sid = self.index_per_contrast[c][idx]
        path = files[fid]
        ...
        mask_seed = self.seed + idx * 1009 + self.epoch * 1000003 + c * 500009
        ...
        # Classical sensitivity: ZF baseline metric ONLY.
        ...
        if self.training:
            ...
            target = rss(ifft2c(full_kspace), dim=0).float()
    ...
    def __getitem__(self, idx):
        ...
        return {
            "komega": torch.stack([d["komega"] for d in per_contrast], dim=0),
            "omega": torch.stack([d["omega"] for d in per_contrast], dim=0),
            "sens": torch.stack([d["sens"] for d in per_contrast], dim=0),
            "target": torch.stack([d["target"] for d in per_contrast], dim=0),
