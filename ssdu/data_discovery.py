"""File discovery, contrast labelling, train/val/test split and coil verification."""
...
def discover_acquisition_labels(files, sample_n=25):
    ...
    for label, n in sorted(counts.items(), key=lambda kv: -kv[1]):
        print(f"  {label!r}: {n}")
    print("Configured CFG.CONTRAST_ACQUISITIONS:", CFG.CONTRAST_ACQUISITIONS)
    print("=" * 80)
...
def get_files_multi_contrast():
    ...
    for label in CFG.CONTRAST_ACQUISITIONS:
        ...
        if len(files) < required:
            raise RuntimeError(
                f"Contrast '{label}': found {len(files)} files, but {required} "
                f"required (TRAIN_FILES + VAL_FILES + TEST_FILES). Lower these "
                f"CFG values, or verify the acquisition label matches your dataset."
            )
