"""Fixed (classical ACS + RSS) and learned (E2E-VarNet style) sensitivity map estimation."""
...
def estimate_sens(kspace, acs_size):
    ...
    sens = torch.complex(
        torch.nan_to_num(sens.real), torch.nan_to_num(sens.imag)
    )
...
class LearnedSensitivityModel(nn.Module):
    ...
    def forward(self, komega):
        ...
        coil_images = extract_acs_images(komega, self.acs_size)
        flat = coil_images.reshape(B * L * C, H, W)

        x = complex_to_channels(flat)
        refined = x + self.refine(x)

        refined_complex = channels_to_complex(refined).reshape(B, L, C, H, W)
        ...
        norm = torch.sqrt(
            torch.sum(torch.abs(refined_complex) ** 2, dim=2, keepdim=True).clamp_min(1e-8)
        )
