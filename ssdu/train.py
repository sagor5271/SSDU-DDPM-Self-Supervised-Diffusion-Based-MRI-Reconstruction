"""Self-supervised validation (held-out Upsilon k-space loss) and training loop."""
...
def validate(model, loader, max_slices):
    ...
    cuda_rng = torch.cuda.get_rng_state(CFG.DEVICE) if torch.cuda.is_available() else None
    ...
    for batch in loader:
        ...
        t = torch.full((B,), CFG.T_STEPS // 2, dtype=torch.long, device=CFG.DEVICE)
    ...
    if torch.cuda.is_available():
        torch.cuda.set_rng_state(cuda_rng, CFG.DEVICE)
...
def train(train_files, val_files):
    ...
    best_val = float("inf")
    best_path = os.path.join(CFG.CHECKPOINT_DIR, "best_ssdu_ddpm_audited.pt")
    ...
    for epoch in range(CFG.EPOCHS):
        ...
        for step_idx, batch in enumerate(train_loader):
            ...
            step_seed = (epoch * 1000003) + (step_idx * 9973) + 777

            out = model.train_step(batch["komega"], batch["omega"], step_seed)
            loss = out["loss"]
        ...
        if CFG.USE_LEARNED_PARTITIONING:
            ...
            for c, label in enumerate(CFG.CONTRAST_ACQUISITIONS):
                print(
                    f"  Learned partition R~ for {label}: {probs[c].mean().item():.4f} "
